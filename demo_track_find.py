...
def separate_color(frame):
    ...
    lower_hsv2 = np.array([11, 70, 60])  # 提取颜色的低值
    high_hsv2 = np.array([34, 255, 70])  # 提取颜色的高值
    lower_hsv = np.array([11, 60, 80])  # 提取颜色的低值
    ...
    mask2=cv2.dilate(mask2, kernel3)
    mask=mask+mask2
    mask=~mask
    return mask/255
...
while(1):
    while(1):
        ...
        mask_yw1 = separate_color(mImgPre)
        mask_yw2 = separate_color(mImg)
        mShow = mask_yw1 * mShow
        mShow = mask_yw2 * mShow
        ...
        for row in range(height - 1):
            for col in range(width - 1):
                if ((row > 20) and (row < (height - 20)) and (col > 20) and (col < (width - 20))):
                    True  # 要执行一句没意义的东西
                else:
                    mShow[row, col] = 0
        kernel3 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        kernel5 = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        mShow = cv2.dilate(mShow, kernel3)
        # mShow = smallTargetFilte(mShow, 10);#小区域归并
        mShow = cv2.erode(mShow, kernel3)
        # 目标离散区域归并
        differ = cv2.morphologyEx(mShow, cv2.MORPH_CLOSE, kernel3)
        ret1, frame, bbox = drawContours(mImg, differ)
        if ret1 == 1:
            break
    ...
    while True:
        ...
        mask=separate_color(frame)
        # Draw bounding box
        if heart(int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3])) and mask[int(bbox[0]+bbox[2]/2),int(bbox[1]+bbox[3]/2)]==1:
            # Tracking success
