...
while(1):
	...
	mask_yw1 = cv2.resize(mask_yw1, (1920, 1080))
	mask_yw2 = cv2.resize(mask_yw2, (1920, 1080))
	...
	mShow = mask_yw2 * mShow
        ...
        mShow = cv2.dilate(mShow, kernel15)
       
        # mShow = smallTargetFilte(mShow, 10);#小区域归并
        
        # 目标离散区域归并
        fps = cv2.getTickFrequency() / (cv2.getTickCount() - timer);
        ret1, frame, bbox = drawContours(mImg, mShow)
        if ret1 == 1:
            break
    ...
    while True:
        ...
        mask=sceolor(frame)
        # Draw bounding box
        if heart(int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3])) and mask[min(int(bbox[1]+bbox[3]/2),1080),min(int(bbox[0]+bbox[2]/2),1920)]==1:
            # Tracking success
