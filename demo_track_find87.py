...
def separate_color(frame):
    ...
    high_hsv = np.array([100, 255, 255])  # 提取颜色的高值
    mask = cv2.inRange(hsv, lowerb=lower_hsv, upperb=high_hsv)  # 下面详细介绍
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 9))
    mask=cv2.dilate(mask,kernel)
    kernel2 = cv2.getStructuringElement(cv2.MORPH_RECT, (35, 35))
    kernel3 = cv2.getStructuringElement(cv2.MORPH_RECT, (35, 35))
...
def heart(x,y,w,h):
    ...
    x_h=x+h/2
    y_h=y+w/2
    if min(abs(x_h),abs(1920-x_h))<10:
        return False
    elif min(abs(y_h),abs(1080-y_h))<10:
        return False
    elif w/h>4 or w/h<0.25:
        return False
    elif w*h>1920*108:
        return False
    else:return True
